...
async function cmdStart(msg, args) {
  ...
  if (refCode?.startsWith('ref_')) {
    ...
    if (referrer && myPID !== refPID) {
      // Write to ref_rewards so game can pick it up (same key as game uses)
      const rr = (await sbGet('ref_rewards')) || {};
      if (!rr[refPID]) rr[refPID] = [];
      // Don't add duplicate
      if (!rr[refPID].find(r => r.from === myPID)) {
        rr[refPID].push({
          from: myPID,
          fromName: name,
          fromTgId: userId,
        ...
        await sbSet('ref_rewards', rr);
      }
    }
  }

  // Build game URL with start_param so WebApp receives it
  const gameUrlWithRef = refCode
    ? `${GAME_URL}?startapp=${encodeURIComponent(refCode)}`
    : GAME_URL;
  ...
  await send(msg.chat.id,
    ...
    `/help — справка`,
    {
      reply_markup: {
        inline_keyboard: [[
          { text: '🚀 НАЧАТЬ ИГРАТЬ', web_app: { url: gameUrlWithRef } }
        ]]
      }
    }
  );
...
async function cmdCollect(msg) {
  ...
  const latestPlayers = await getPlayers();
  const latest = latestPlayers[pid];
  if (!latest) {
    return send(msg.chat.id, '❌ Профиль не найден. Зайди в игру ещё раз.', { reply_markup: btnOpenGame() });
  }

  const latestLastCollect = latest.lastBotCollect || latest.lastSeen || Date.now();
  const latestHoursGone   = Math.min(24, (now - latestLastCollect) / 3600000);
  if (latestHoursGone < 0.25) {
    const minLeft = Math.ceil((0.25 - latestHoursGone) * 60);
    return send(msg.chat.id, `⏳ Сбор уже был недавно. Подожди ещё ${minLeft} мин.`);
  }

  const latestBaseInc = (latest.props || []).reduce((s, id) => s + (PROP_INCOMES[id] || 0), 0);
  const latestEarned  = Math.floor(latestBaseInc * latestHoursGone);
  if (latestEarned <= 0) {
    return send(msg.chat.id,
      '🏚️ У тебя нет бизнеса — нечего собирать.\nКупи бизнес в игре!',
      { reply_markup: btnOpenGame('🏢 Открыть Бизнес') }
  ...
  latest.money          = (latest.money || 0) + latestEarned;
  latest.lastBotCollect = now;
  latestPlayers[pid]    = latest;
  await sbSet('players', latestPlayers);
